...
def make_predictions(model, processed_img):
...
def interpret_prediction(model, processed_img, target):
    interprediction_algo = IntegratedGradients(model)
    feature_imp = interprediction_algo.attribute(processed_img.unsqueeze(0), target=int(target))
...
if upload:
    ...
    probs, idxs = make_predictions(model, preprocessed_img)
    ...
    plt.barh(y=categories[idxs], width=probs, color="tomato")
    plt.title("Top 5 Probabilites", loc = "center", fontsize=15)
